//
//  UICollectionViewCellBinding.swift
//  Sugo
//
//  Created by 陈宇艺 on 2019/7/1.
...
class UICollectionViewCellBinding: CodelessBinding {
        ...
        if !running && swizzleClass != nil {
            let executeBlock = {
                ...
                if let root = UIApplication.shared.keyWindow {
                    ...
                    var strPath : String = ""
                    for item in array {
                        strPath = String(format: "%@%@", "/",item)
                    }
                    self.path.string = strPath
                    
                    let sectionCell : String = array[array.count-1]
                    var cellArray = sectionCell.components(separatedBy: "[")
                    var row : Int = 0
                    if cellArray.count <= 1 {
                        row = indexPath.row
                    }else{
                        row =  Int(cellArray[1].components(separatedBy: "]")[0])!
                    }
                    // select targets based off path
                    if self.path.isUITableviewCellLeafSelected(leaf: collectionView, root: root, finalPredicate:true, num:1) && row == indexPath.row {
                        var p = Properties()
